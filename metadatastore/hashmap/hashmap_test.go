...
func TestGetByPath(t *testing.T) {
	...
	p2 := "/path/to/file2"
	s.Add(&entity.Photo{FileInfo: &fs.FileInfo{Path: p}})
	s.Add(&entity.Photo{FileInfo: &fs.FileInfo{Path: p2}})
	r, _ := s.GetByPath(p)
	if len(r) != 1 {
		t.Errorf("Expected one result, got %v", len(r))
	}
...
func TestGetByMonth(t *testing.T) {
	...
	s.Add(&entity.Photo{FileInfo: &fs.FileInfo{Path: p}, Metadata: &entity.Metadata{CreatedAtMonth: m}})
	r, _ := s.GetByMonth(m)
...
func TestGetMonths(t *testing.T) {
	...
	s.Add(&entity.Photo{FileInfo: &fs.FileInfo{Path: p}, Metadata: &entity.Metadata{CreatedAtMonth: m}})
	s.Add(&entity.Photo{FileInfo: &fs.FileInfo{Path: p2}, Metadata: &entity.Metadata{CreatedAtMonth: m2}})
	r, _ := s.GetMonths()
	if len(r) != 2 {
