...
import (
	...
	"github.com/marpio/img-store/fileutils"
...
func TestGetAll(t *testing.T) {
	s, _ := setup()
	p := "/path/to/file"
	s.Add(&entity.Photo{FileInfo: &fileutils.FileInfo{Path: p}})
	r, _ := s.GetAll()
	if len(r) != 1 {
...
func TestGetByPath(t *testing.T) {
	...
	p2 := "/path/to/file2"
	s.Add(&entity.Photo{FileInfo: &fileutils.FileInfo{Path: p}})
	s.Add(&entity.Photo{FileInfo: &fileutils.FileInfo{Path: p2}})
	r, _ := s.GetByPath(p)
	if len(r) != 1 {
		t.Errorf("Expected one result, got %v", len(r))
	}
...
func TestGetByMonth(t *testing.T) {
	...
	s.Add(&entity.Photo{FileInfo: &fileutils.FileInfo{Path: p}, Metadata: &entity.Metadata{CreatedAtMonth: m}})
	r, _ := s.GetByMonth(m)
...
func TestGetMonths(t *testing.T) {
	...
	s.Add(&entity.Photo{FileInfo: &fileutils.FileInfo{Path: p}, Metadata: &entity.Metadata{CreatedAtMonth: m}})
	s.Add(&entity.Photo{FileInfo: &fileutils.FileInfo{Path: p2}, Metadata: &entity.Metadata{CreatedAtMonth: m2}})
	r, _ := s.GetMonths()
	if len(r) != 2 {
